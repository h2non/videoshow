...
function renderVideo(videoshow, options, forwardEvent, cb) {
  ...
  copy(output, tempfile, function (err) {
    ...
    if (subtitles) {
      ...
      if (params.subtitleOptions) {
        Object.keys(params.subtitleOptions).forEach(key => {
          subtitleOptions.push(key + '=' + '\'' + params.subtitleOptions[key] + '\'');
        });
      }
      console.log({ subtitleOptions });
      options.push(`-vf subtitles=` + subtitleOptions.join(':'))
    }
...
function getImageSubtitles(videoshow) {
  ...
  if (params.useSubRipSubtitles) {
    ...
    subtitles = substation.stringify(subtitles, {
      'V4 Styles': {
